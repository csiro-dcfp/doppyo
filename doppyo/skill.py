...
def rank_histogram(da_cmp, da_ref, over_dims, norm=True, ensemble_dim='ensemble'):
    """
        ...
        Parameters
        ----------
        da_cmp : xarray.DataArray or xarray.Dataset
        ...
        da_ref : xarray.DataArray or xarray.Dataset
        ...
        Returns
        -------
        rank_histogram : xarray.DataArray or xarray.Dataset
    ...
    # Rank the data -----
    if dask.is_dask_collection(combined):
    ...
    if norm:
        res = utils.pdf(da_ranked, bin_edges, over_dims=over_dims)
    else:
        res = utils.histogram(da_ranked, bin_edges, over_dims=over_dims)
    res.coords['metric'] = 'rank_histogram'
    return res
...
def rps(da_cmp, da_ref, bins, over_dims=None, ensemble_dim='ensemble'):
    """
        ...
        Returns
        -------
        rps : xarray.DataArray or xarray.Dataset
    ...
    res = utils.integrate((cdf_cmp - cdf_ref) ** 2, over_dim='bins') \
                .mean(dim=over_dims, skipna=True)
    res.coords['metric'] = 'rps'
    return res
...
def reliability(cmp_likelihood, ref_logical, over_dims, probability_bins=np.linspace(0,1,5),
    ...
    """
        ...
        Returns
        -------
        reliability : xarray.DataArray or xarray.DataSet
    ...
    # TODO: find nicer name for new dimension
    reliability = xr.concat([relative_freq, cmp_number, ref_occur], 'component')
    reliability['component'] = ['relative_freq', 'cmp_number', 'ref_occur']
...
def roc(cmp_likelihood, ref_logical, over_dims, probability_bins=np.linspace(0,1,5)):
    ...
    for idx,probability_bin_edge in enumerate(probability_bin_edges):
        ...
        contingency_table = contingency(cmp_likelihood, ref_binary,
                                                category_edges, category_edges, over_dims=over_dims)

        # Add hit rate and false alarm rate to lists -----
        hit_rate_list.append(hit_rate(contingency_table,yes_category=2))
        false_alarm_rate_list.append(false_alarm_rate(contingency_table,yes_category=2))

    # Concatenate lists -----
    h_r = xr.concat(hit_rate_list, dim='probability_bin')
    h_r['probability_bin'] = probability_bins
    f_a_r = xr.concat(false_alarm_rate_list, dim='probability_bin')
    f_a_r['probability_bin'] = probability_bins

    # Calculate area under curve -----
    dx = f_a_r - f_a_r.shift(**{'probability_bin':1})
    dx = dx.fillna(0.0)
    area = abs(((h_r.shift(**{'probability_bin':1}) + h_r) * dx / 2.0) \
    ...
    # package in new dimension component
    roc = xr.concat([h_r, f_a_r, area],'component', coords='minimal', compat='override')
    roc['component'] = ['hit_rate', 'false_alarm_rate', 'area']
    roc.coords['metric']='roc'
...
def discrimination(cmp_likelihood, ref_logical, over_dims, probability_bins=np.linspace(0,1,5)):
    ...
    # package to dimension component
    discrimination = xr.concat([hist_event, hist_no_event],'component')
    discrimination['component'] = ['hist_event','hist_no_event']
    discrimination.coords['metric'] = 'discrimination'
...
def Brier_score(cmp_likelihood, ref_logical, over_dims, probability_bins=None):
    ...
    Brier.coords['metric'] = 'Brier_score'
    ...
    if probability_bins is not None:
        ...
        # package as component dimension
        Brier = xr.concat([Brier_total, Brier_reliability, Brier_resolution, Brier_uncertainty], 'component')
        Brier['component'] = ['total', 'reliability', 'resolution', 'uncertainty']
...
def contingency(da_cmp, da_ref, category_edges_cmp, category_edges_ref, over_dims):
    """
        ...
        Returns
        -------
        contingency : xarray.DataArray or xarray.Dataset
    ...
    if len(da_list) == 1:
        da_list[0].coords['metric']='contingency'
        return da_list[0]
    else:
        res = xr.concat(da_list, dim='stack') \
                 .set_index(stack=['comparison_category', 'reference_category']) \
                 .unstack('stack')
        res.coords['metric'] = 'contingency'
        return res
...
def accuracy_score(contingency):
    """
        ...
        Returns
        -------
        accuracy_score : xarray.DataArray or xarray.Dataset
    ...
    res = (hits / N)
    res.coords['metric'] = 'accuracy_score'
    return res
...
def Heidke_score(contingency):
    """
        ...
        Returns
        -------
        Heidke_score : xarray.DataArray or xarray.Dataset
    ...
    res =  ((numer_1 - numer_2) / denom)
    res.coords['metric'] = 'Heidke_score'
    return res
...
def Peirce_score(contingency):
    """
        ...
        Returns
        -------
        Peirce_score : xarray.DataArray or xarray.Dataset
    ...
    res = ((numer_1 - numer_2) / denom)
    res.coords['metric'] = 'Peirce_score'
    return res
...
def Gerrity_score(contingency):
    """
        ...
        Returns
        -------
        Gerrity_score : xarray.DataArray or xarray.Dataset
    ...
    res = ((contingency * S).sum(dim=('reference_category','comparison_category'), skipna=True) / \
            _sum_contingency(contingency, 'total'))
    res.coords['metric'] = 'Gerrity_score'
    return res
...
def bias_score(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        bias_score : xarray.DataArray or xarray.Dataset
    ...
    res = ((hits + false_alarms) / (hits + misses))
    res.coords['metric'] = 'bias_score'
    return res
...
def hit_rate(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        hit_rate : xarray.DataArray or xarray.Dataset
    ...
    res = (hits / (hits + misses))
    res.coords['metric'] = 'hit_rate'
    return res
...
def false_alarm_ratio(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        false_alarm_ratio : xarray.DataArray or xarray.Dataset
    ...
    res = (false_alarms / (hits + false_alarms))
    res.coords['metric'] = 'false_alarm_ratio'
    return res
...
def false_alarm_rate(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        false_alarm_rate : xarray.DataArray or xarray.Dataset
    ...
    res = (false_alarms / (correct_negs + false_alarms))
    res.coords['metric'] = 'false_alarm_rate'
    return res
...
def success_ratio(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        success_ratio : xarray.DataArray or xarray.Dataset
    ...
    res = (hits / (hits + false_alarms))
    res.coords['metric'] = 'success_ratio'
    return res
...
def threat_score(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        threat_score : xarray.DataArray or xarray.Dataset
    ...
    res = (hits / (hits + misses + false_alarms))
    res.coords['metric'] = 'threat_score'
    return res
...
def equit_threat_score(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        equit_threat_score : xarray.DataArray or xarray.Dataset
    ...
    res = ((hits - hits_random) / (hits + misses + false_alarms + hits_random))
    res.coords['metric'] = 'equit_threat_score'
    return res
...
def odds_ratio(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        odds_ratio : xarray.DataArray or xarray.Dataset
    ...
    res = ((hits * correct_negs) / (misses * false_alarms))
    res.coords['metric'] = 'odds_ratio'
    return res
...
def odds_ratio_skill_score(contingency, yes_category=2):
    """
        ...
        Returns
        -------
        odds_ratio_skill_score : xarray.DataArray or xarray.Dataset
    ...
    res.coords['metric'] = 'odds_ratio_skill'
    return res
...
def mean_additive_bias(da_cmp, da_ref, over_dims):
    """
        ...
        Returns
        -------
        mean_additive_bias : xarray.DataArray or xarray.Dataset
    ...
    res = (da_cmp - da_ref).mean(dim=over_dims, skipna=True)
    res.coords['metric'] = 'mean_additive_bias'
    return res
...
def mean_multiplicative_bias(da_cmp, da_ref, over_dims):
    """
        ...
        Returns
        -------
        mean_multiplicative_bias : xarray.DataArray or xarray.Dataset
    ...
    res = (da_cmp / da_ref).mean(dim=over_dims, skipna=True)
    res.coords['metric'] = 'mean_multiplicative_bias'
    return res
...
def mean_absolute_error(da_cmp, da_ref, over_dims):
    """
        ...
        Returns
        -------
        mean_absolute_error : xarray.DataArray or xarray.Dataset
    ...
    res = abs(da_cmp - da_ref).mean(dim=over_dims, skipna=True)
    res.coords['metric'] = 'mean_absolute_error'
    return res
...
def mean_squared_error(da_cmp, da_ref, over_dims):
    """
        ...
        Returns
        -------
        mean_squared_error : xarray.DataArray or xarray.Dataset
    ...
    res = xr.ufuncs.square(da_cmp - da_ref).mean(dim=over_dims,skipna=True)
    res.coords['metric'] = 'mean_squared_error'
    return res
...
def rms_error(da_cmp, da_ref, over_dims):
    """
        ...
        Returns
        -------
        rms_error : xarray.DataArray or xarray.Dataset
    ...
    res = xr.ufuncs.sqrt(mean_squared_error(da_cmp, da_ref, over_dims=over_dims))
    res.coords['metric'] = 'root_mean_squared_error'
    return res
...
def Pearson_corrcoeff(da_cmp, da_ref, over_dims, subtract_local_mean=True):
    """
        ...
        Returns
        -------
        Pearson_corrcoeff : xarray.DataArray or xarray.Dataset
    ...
    res = (cov / norm).mean(difference_dims)
    res.coords['metric'] = 'Pearson_corrcoeff'
    return res
